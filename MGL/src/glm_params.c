...
void getiOSDefaults(GLMContext glm_ctx)
{
    ...
    void *dl_handle = dlopen("@rpath/libEGL.framework/libEGL", RTLD_LAZY | RTLD_LOCAL);
    void *gl = dlopen("@rpath/libGLESv2.framework/libGLESv2", RTLD_LAZY | RTLD_LOCAL);
    assert(dl_handle);
    assert(gl);

    eglBindAPI               = dlsym(dl_handle, "eglBindAPI");
    eglChooseConfig          = dlsym(dl_handle, "eglChooseConfig");
    eglCreateContext         = dlsym(dl_handle, "eglCreateContext");
    eglCreateWindowSurface   = dlsym(dl_handle, "eglCreateWindowSurface");
    eglDestroyContext        = dlsym(dl_handle, "eglDestroyContext");
    eglDestroySurface        = dlsym(dl_handle, "eglDestroySurface");
    eglGetConfigAttrib       = dlsym(dl_handle, "eglGetConfigAttrib");
    eglGetConfigs            = dlsym(dl_handle, "eglGetConfigs");
    eglGetCurrentContext     = dlsym(dl_handle, "eglGetCurrentContext");
    eglGetCurrentSurface     = dlsym(dl_handle, "eglGetCurrentSurface");
    eglGetDisplay            = dlsym(dl_handle, "eglGetDisplay");
    eglGetError              = dlsym(dl_handle, "eglGetError");
    eglGetPlatformDisplay    = dlsym(dl_handle, "eglGetPlatformDisplay");
    eglInitialize            = dlsym(dl_handle, "eglInitialize");
    eglMakeCurrent           = dlsym(dl_handle, "eglMakeCurrent");
    eglQueryString           = dlsym(dl_handle, "eglQueryString");
    eglReleaseThread         = dlsym(dl_handle, "eglReleaseThread");
    eglSwapBuffers           = dlsym(dl_handle, "eglSwapBuffers");
    eglSwapInterval          = dlsym(dl_handle, "eglSwapInterval");
    eglTerminate             = dlsym(dl_handle, "eglTerminate");
